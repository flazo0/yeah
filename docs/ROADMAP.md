...
- Trocar o padrão `activeTab`/`activeConfigTab` (refs locais com `v-if`) por **rotas filhas de verdade no Vue Router** (`/apps/:id/domains`, `/apps/:id/env`, `/apps/:id/storage`, `/apps/:id/deployments`, etc.), com um componente de layout compartilhado (breadcrumb + tabs + `<router-view>`) — mesmo padrão em Application/Database/Service. **Application já migrado** (`ApplicationLayout.vue` + `pages/application/*`, dados compartilhados via `provide`/`inject` em `useApplicationContext`) — testado de ponta a ponta: clique de aba muda a URL, voltar/avançar do navegador funciona, reload direto numa sub-rota (ex. `/env`) renderiza certo. Database e Service ainda usam o `activeTab` antigo — próximo passo é repetir o mesmo padrão neles.
