...
- [x] **Backup de volume/storage persistente** de **aplicações**: botão por volume (volume Docker, diretório do servidor, arquivo), `.tar.gz` no servidor (5 mais recentes por volume), baixar, restaurar (para a app, substitui, reinicia) e excluir; testado com os 3 tipos, arquivos ocultos, retenção e isolamento entre aplicações. **Serviços em stack** também (aba Backups: cada volume nomeado do compose, restore para os containers da stack, mesma retenção). **Envio pro S3** (destino escolhido por backup, restore e exclusão a partir do S3, retenção também remove o objeto) — **implementado, não testado** (só typecheck e testes unitários; testar numa VPS real com um bucket).
- [x] Aviso explícito na UI: "volume persistente não é backup" (aba Armazenamento das aplicações). Também na aba Backups dos serviços. Falta nos bancos.
