...
- **✅ Feito**: dashboard inicial de verdade ao logar, no estilo "Root Team" do Coolify — cards de contagem (aplicações/bancos/serviços/servidores), deploys recentes cross-projeto (`GET /teams/:teamId/overview`), grade de "Projetos" (preview + "Ver todos") e grade de "Servidores" com status/CPU/RAM/disco ao vivo, tudo pro time principal do usuário. **Trocar/criar time saiu do corpo do dashboard** e virou um dropdown no topbar (`TeamSwitcher.vue`, substituindo o nome do time no breadcrumb) — igual o "Root Team ⌄" do Coolify: lista os times, marca o atual, e tem "Novo time" com formulário inline, tudo discreto em vez de um card full-width. Falta ainda: gráfico histórico de métricas (depende de série temporal — ver seção Robustez) e widget de traffic analytics (baixa prioridade, ver achado abaixo).
- **✅ Feito**: componentes de configuração **compartilhados entre Application/Database/Service** — `DomainCard.vue` e `ResourceLimitsCard.vue` (`apps/web/src/components/`), reaproveitados pelas três páginas "Geral" em vez de cada uma reimplementar o próprio formulário. Variáveis de ambiente e tags ainda não têm componente compartilhado (cada recurso só tem env vars hoje; tags nem existem ainda).
- **✅ Feito**: sidebar reorganizada em seções (Infraestrutura / Integrações / Sistema) em vez de uma lista única — mais perto da divisão Workspace/Infrastructure/Manage do Coolify.
- **✅ Feito**: `EnvironmentPage.vue` (a tela "Recursos" de um ambiente) unificada — antes tinha três grades separadas (Aplicações/Bancos/Serviços) e três formulários de criação sempre visíveis embaixo; agora é uma grade só, mista, ordenada por nome, e criar recurso virou um fluxo dedicado (`ResourceNewPage.vue`, rota `.../new`) com um seletor de 3 categorias (Aplicação/Banco de dados/Serviço) — igual o catálogo "New Resource" do Coolify, sem forms permanentes ocupando a tela.
- **✅ Feito**: tema escuro virou o padrão no primeiro acesso (sem preferência salva ainda), igual Coolify e a maioria dos painéis self-hosted — antes seguia `prefers-color-scheme` do SO. O toggle continua funcionando normalmente e o que for escolhido lá passa a persistir. Paleta escura também recalibrada: era preto puro (`#000`/`#0a0a0a`) numa escala só, virou um sistema de camadas (fundo → chrome → card → hover) mais próximo do "Graphite" real do Coolify, com mais profundidade percebida entre sidebar/topbar/card.
...
- Ainda falta: página do GitHub (`GithubPage.vue`) reestruturada no estilo "Sources" do Coolify.
