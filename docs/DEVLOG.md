...
## Fase 3 (parte 4): tarefas agendadas

Fiz as tarefas agendadas de uma aplicação: comando rodado com `docker exec <container> sh -c '<comando>'` num cron. Tabelas `scheduled_tasks` e `scheduled_task_executions` (migração 0020), fila `scheduled-task` com um job scheduler do BullMQ por tarefa (chave = id da tarefa: editar troca o cron no lugar, pausar remove), job no worker com limite de tempo próprio (estourou → derruba a conexão SSH e marca `failed`), log truncado em 200 KB, histórico nas últimas 50 execuções e notificação `task.failed`. Rotas em `/applications/:id/tasks` (CRUD, `/run`, `/executions`); cron ou fuso inválido dá 400 e a linha é desfeita. Aba "Tarefas agendadas" na aplicação, com presets de cron, histórico e polling de 3s.
...
## Fase 3 (parte 6): Docker Compose como build pack

Uma aplicação agora pode ser um projeto Docker Compose: build pack `docker_compose` (migração 0023: valor novo no enum, `compose_file` e `compose_service`). O deploy clona o repositório, grava o `.env` (com as variáveis `build:` também, porque o compose interpola `${VAR}` no build) em `repo/.env` e no diretório da app e roda `docker compose -p yeah-app-<id> --env-file … -f <arquivo> up -d --build --remove-orphans --wait`. Com domínio e serviço escolhido, escrevo um override (`compose.yeah.override.yml`) que põe só aquele serviço na rede do proxy com os labels do Traefik — o compose junta labels e redes dos dois arquivos, então o arquivo do usuário não é tocado. Como não existe mais "o container", tudo que endereçava `yeah-app-<id>` passou a usar os labels `com.docker.compose.project/service`: logs (todos os containers, cada linha prefixada), start/stop/restart, exec das tarefas agendadas e a exclusão (containers, rede e volumes do projeto). Helpers puros em `packages/shared/src/compose.ts` com testes; validação de caminho do arquivo e nome do serviço na criação. Card "Docker Compose" no catálogo e campos no modal; a aba Geral mostra arquivo e serviço.

**Testado** (painel local + "VPS" docker-in-docker com Compose v2.33, repositório git local com 2 serviços — `web` nginx com healthcheck e volume, `worker` construído de um Dockerfile): arquivo `../`/absoluto e serviço com `;` → 400; deploy `success` com os dois containers no ar (`web` healthy), o build do worker rodou (`/built.txt`), `GREETING` chegou no container, `build:SECRET_BUILD` está no `.env` do compose; no `web` os labels `Host(compose.example.test)` e porta 80 e as redes do projeto + `yeah-proxy-net`; o `worker` sem labels do Traefik; aba de logs com as linhas de ambos; parar → status `stopped` e 0 containers rodando, iniciar e reiniciar → `running` com 2; uma tarefa agendada rodou no `web` e viu a variável; redeploy idempotente e o volume nomeado do projeto preservado; um compose que sai com código 3 fez o deploy `failed` ("exited (3)") e a aplicação `error`; excluir a aplicação removeu containers, rede, volume e diretório e deixou `yeah-proxy-net` intacta. Na tela: card, campos do modal, criação e a aba Geral com arquivo/serviço. 214 testes, `tsc` e `vue-tsc` passam.

**Achado no caminho**: parar leva os 10 s de tolerância se algum container ignora SIGTERM (meu `sleep` como PID 1) — comportamento esperado, mas o status só vira `stopped` depois disso.

**Não testado**: Traefik de verdade roteando (só conferi labels e rede — o proxy não foi subido na VPS de teste), repositório privado por deploy key com compose, compose com `env_file` apontando pro `.env`, Compose < 2.17 (o `--wait` não existe lá).

## Fase 3 (parte 7): terminal, origem/servidor/operações, domínios, mudanças pendentes, env em tabela

**Terminal interativo.** Rota WebSocket na API (`/servers/:id/terminal` e `/applications/:id/terminal`): abre SSH, pede um PTY (`shell()` no servidor, `exec("docker exec -it …")` no container) e repassa bytes nos dois sentidos; o protocolo do navegador é JSON (`input` / `resize`) validado por `parseTerminalMessage`. Como WebSocket não é protegido por CORS, a rota confere a Origin contra `WEB_ORIGIN` além do cookie de sessão e da participação no time; 5 sessões por usuário e 30 min de inatividade. O componente xterm (`TerminalView`) substituiu o "preview local" que só ecoava teclas. O nginx (padrão e o gerado pelo `PANEL_PATH`) ganhou upgrade de WebSocket em `/api/` (`nginx -t` nas duas variantes).

**Achado importante**: o terminal ficava eternamente em "conectando…" no navegador porque o CSP do `index.html` só liberava `http://localhost:3000` em `connect-src` e o CSP trata `ws://` como outro esquema — o clique falhava em 1 ms, sem nada no servidor. Agora o vite injeta a URL da API em `ws://` no CSP (em produção a API é same-origin e o `'self'` já cobre).

**Domínios.** `extra_domains` + `www_redirect` (migração 0024). `packages/shared/src/traefik.ts` gera as labels (regra `Host() || Host()`, e o redirect como segundo router com middleware `redirectregex`, permanente); o `docker run` e o override do compose usam a mesma função (no compose o `$` vira `$$`, porque o compose interpola). Na API: hostnames validados, no máximo 10, e conflito com outra app do time devolve 409 — inclusive com o www reservado pelo redirect de outra (achei que isso passava em branco no teste e consertei).

**Mudanças pendentes.** `deployed_config` (jsonb) recebe o snapshot no início de cada deploy; a API devolve `pendingChanges` comparando com a linha atual. **Origem / Servidor / Operações**: rotas novas em `applicationOps.ts`, com o teardown extraído pra `lib/appTeardown.ts` (usado também na exclusão). **Env em tabela**: `serializeEnvEntries` é o inverso do parser.

**Testado** (painel local + "VPS" docker-in-docker): terminal — origem errada/ausente 4403, sem cookie ou cookie inválido 4401, servidor de outro time 4404, **shell do servidor** (`echo hello-$((1+2))` voltou `hello-3`, `stty size` depois de um resize deu `40 120`), `exit` fecha com 1000, terminal do container antes do deploy mostra o erro do docker e depois do deploy entra no Alpine, 6º terminal aberto ao mesmo tempo é recusado (4429) e volta a abrir quando os outros fecham. Domínios/pendências: validação (400), conflito (409), labels no container real (`${1}` intacto), `pendingChanges` nulo → `[]` após o deploy → lista certa depois de editar env/domínio/volume → `[]` após redeploy, origem (imagem/porta/branch/pasta inválidas 400, salvar ok), clone (sem domínio nem token, volume copiado, tarefa pausada, ambiente de outro time 404), mover entre ambientes, trocar de servidor (container e diretório removidos do antigo, app `idle`, redeploy no novo sobe). Na tela: tabela de env (nome inválido marcado, salvar gravou `both:FROM_TABLE=v1`), banner de pendência, as abas Origem/Servidor/Operações e o card de domínios renderizam; o navegador abriu o WebSocket do terminal e recebeu o prompt do container. 239 testes, `tsc` e `vue-tsc` passam.

**Não testado**: o **xterm desenhando** e aceitando teclas — o navegador de automação ficou com viewport de largura 0 e não renderiza; só confirmei a conexão e o prompt recebido, e o protocolo de ponta a ponta com um cliente WebSocket real. Traefik de verdade aplicando os redirects. Mover uma app entre times (não existe). Terminal de banco/serviço.

## Fase 3 (parte 8): preview deployments

Uma aplicação ligada ao GitHub pode ativar previews (`preview_enabled`, migração 0025). Um preview é uma linha `applications` comum copiada da principal (`previewOfId` + `prNumber`): por isso reaproveita deploy, logs, terminal e exclusão sem código novo. `handlePullRequestEvent` (`apps/api/src/lib/previews.ts`) trata o webhook `pull_request`: só age em repositório/branch-base que batem com uma app com previews ligados, **ignora PR de fork** (rodaria código de fora ao lado das variáveis da app), exige proxy ativo + wildcard (cada preview ganha `pr-N-<app>.<wildcard>`), cria ou atualiza a linha, enfileira o deploy; `closed` apaga a linha e derruba o container em segundo plano (o GitHub dá ~10 s pro webhook responder). O `push` na branch do PR já não dispara deploy do preview (o `pull_request.synchronize` faz isso; senão seriam dois por push). O worker comenta a URL no PR com um marcador escondido, então há um comentário por preview, editado a cada push (`upsertPullRequestComment`, testado com fetch simulado). Excluir a app principal derruba os previews. O clone de aplicação foi refatorado pra usar o mesmo `applicationCopyValues`.

**Achado no caminho (bug real, não do preview)**: se o token do GitHub falhava (App desinstalado, `GITHUB_APP_*` ausente), a exceção saía **antes** do `try` do deploy e o deployment ficava `running` pra sempre, com a aplicação em `deploying`. Movi a montagem da URL de clone pra dentro do `try` — agora o deploy falha na hora com a mensagem certa (verificado).

**Testado** (API com `GITHUB_APP_*` de teste + VPS docker-in-docker, webhooks assinados com HMAC de verdade): assinatura errada 401; fork, outra branch-base, ação `labeled` ignorados; `opened` cria a cópia (nome, branch do PR, domínio `pr-7-prev-app.apps.test`, env copiada, volumes com ids distintos) e enfileira o deploy; `synchronize` mantém a mesma linha e enfileira outro; `push` na branch do PR não dispara; os deploys terminaram `success` com o container no ar, a regra do roteador `Host(pr-7-prev-app.apps.test)` e o conteúdo **da branch do PR** (`preview7`) sendo servido, sem container da principal; `closed` removeu a linha, o container e o diretório, deixou a principal intacta e um segundo `closed` foi `skipped`; excluir a principal com um preview aberto derrubou os dois. Na tela: a aba Previews renderiza e ativar numa app sem GitHub mostra o erro certo. 248 testes, `tsc` e `vue-tsc` passam.

**Não testado**: comentário real no PR (precisa de um GitHub App com permissão em pull requests), Traefik roteando de verdade, um preview de repositório privado (a URL de clone com token é a mesma do deploy normal, mas não passei por ela).

## Fase 3 (parte 9): armazenamento por tipo, métricas do container e Railpack

**Armazenamento.** `application_volumes` ganhou `kind` (volume | bind | file), `host_path` e `file_content` (criptografado como as variáveis) — migração 0026. `volumeFlags` monta o `-v` certo por tipo; um volume de arquivo é escrito pelo worker em `/opt/yeah-apps/<app>/files/<id>` antes do `docker run` e montado por cima. Caminhos validados (`isSafeHostPath`: absoluto, sem `..`, sem caractere de shell, nunca `/`; `isSafeMountPath`). O snapshot de "mudanças pendentes" passou a considerar tipo, origem e o hash do conteúdo. Clone e preview copiam os três tipos.

**Métricas.** `GET /applications/:id/metrics` roda `docker stats --no-stream --format '{{json .}}'` por SSH (mesma exceção de leitura curta dos logs) e `parseDockerStats` converte unidades (kB/MiB/GiB…) em bytes. A aba desenha CPU e memória com um SVG de sparkline e atualiza a cada 5 s só enquanto está aberta.

**Railpack** entrou como build pack irmão do Nixpacks (`railpack build <repo> --name <imagem> --env …`).

**Testado** (painel local + "VPS" docker-in-docker): validação (mount relativo/com aspas, bind sem diretório, com `..` ou `$(id)`, arquivo sem conteúdo → 400); um volume de cada tipo: o conteúdo do arquivo chega **byte a byte** no container (com `$dollar` e aspas) e o nginx o serve, guardado como `enc:v1:` no banco; o bind lê um arquivo criado no host e o que o container escreve aparece no host; editar o arquivo e redeployar troca o conteúdo enquanto o volume nomeado mantém o dado; editar o conteúdo de um volume não-arquivo → 400; as mudanças pendentes acusaram "Armazenamento persistente"; métricas reais (CPU, memória usada e limite > 0, 9 processos); o clone manteve `volume`, `bind` (com o caminho) e `file` (com conteúdo); a exclusão da app apagou containers e diretório. Na tela: aba Métricas com 2 gráficos vivos, aba Armazenamento com o seletor de tipo, o campo de arquivo aparece e o aviso "volume persistente não é backup". 263 testes, `tsc` e `vue-tsc` passam.

**Não testado**: Railpack de verdade (sem internet nem `curl` na VPS de teste — só a falha visível), o gráfico de métricas com muitas amostras, arquivos de volume em aplicações Docker Compose (não usam essa lista).

## Fase 3 (parte 10): registry privado com push por commit

Tabela `registries` (senha criptografada, migração 0027) e, na aplicação, `registry_id` + `registry_image`. O deploy: escreve a senha num arquivo 600 na VPS, faz `docker login … --password-stdin < arquivo` (o arquivo é apagado em qualquer resultado; nada de `-p` na linha de comando, que o `ps` do servidor veria) e, no fim, `docker logout`. Pra app construída com repositório configurado: a tag é o commit (12 caracteres) mais, se há variáveis de build, 6 caracteres do hash delas — mudar um build arg nunca reaproveita imagem velha. Se a tag já existe, o worker só baixa e renomeia; senão constrói, marca e envia. Rollback cai no mesmo caminho (o commit alvo já tem tag).

**Achado no caminho**: minha primeira checagem de existência (`docker manifest inspect`) não respeita registries HTTP/inseguros configurados no daemon e sempre dava "não existe" — o push funcionava mas o reuso nunca. Troquei por um `docker pull -q` silencioso, que segue as regras do daemon (e o pull posterior nunca discorda da checagem).

**Testado** (painel local + registry:2 na VPS docker-in-docker): endereço/usuário/senha inválidos 400, nome duplicado 409, a URL colada com `https://…/` vira `localhost:5000`, a senha não sai em nenhuma resposta e fica `enc:v1:` no banco, editar sem senha mantém a guardada; sem repositório 400, repositório com maiúsculas 400, registry de outro time 404; deploy 1 login → build → push (`e2e/site:<commit>` na lista do registry, site servido); **mesmo commit: "reusando a imagem já construída", sem build**; commit novo: build + push de outra tag; **rollback pro primeiro deploy: sem build e o conteúdo voltou pra v1**; mudar `build:VAR` gerou a tag com sufixo e construiu de novo; o arquivo da senha sumiu e o `config.json` do docker não guarda o registry depois; app de imagem só faz login antes do pull; excluir o registry limpa o vínculo das apps. 270 testes, `tsc` e `vue-tsc` passam.

**Não testado**: registry real com HTTPS e credencial (Docker Hub, GHCR), login com senha errada (o registry local não exige auth), a tela Registries no navegador (só o typecheck), limpeza de tags antigas no registry (não existe ainda).

## Fase 3 (parte 11): fontes GitLab, Bitbucket e Gitea

`git_sources` (provedor, URL base, usuário, token e segredo de webhook criptografados, migração 0028) e, na aplicação, `git_source_id` + `git_repo` ("grupo/projeto"). O worker monta a URL de clone com o token só na hora do deploy (`gitCloneUrlWithToken`: GitLab usa `oauth2`, Gitea e Bitbucket o usuário; usuário e token vão percent-encoded, então um token com `@`, `/` ou `#` não quebra a URL) e, **logo depois do clone, volta o `origin` pra URL sem credencial** — sem isso o token ficaria em `.git/config` na VPS pra sempre (o mesmo vale agora pro token de 1 h do GitHub App). Listagem de repositórios pela API de cada provedor (`parseRepoList` com o formato documentado de cada um). Webhook único por fonte (`/webhooks/git/<id>`): GitLab compara o token do cabeçalho, Gitea/Forgejo e Bitbucket validam HMAC-SHA256; "fonte inexistente" e "segredo errado" respondem o mesmo 401 (o endpoint não confirma quais ids existem), e o segredo de uma fonte não vale em outra.

**Testado** (painel local + VPS docker-in-docker; API dos provedores simulada num servidor local; um servidor git por HTTP na VPS pra clonar): validação (provedor inválido, Gitea sem URL ou sem usuário, URL com credenciais, sem token → 400; nome duplicado 409); token e segredo `enc:v1:` no banco e fora de qualquer resposta; a listagem mandou `Authorization: token …` pro Gitea e `PRIVATE-TOKEN` pro GitLab e devolveu os repositórios; provedor inalcançável → 502; criar app pela fonte (sem repositório, com `../etc`, fonte inexistente, misturada com deploy key → 400/404) guarda a URL **sem** token; **deploy clonou e o nginx serviu o conteúdo do repositório**, `origin` no servidor é a URL limpa, o token não está no `.git/config` nem no log do deploy; trocar o repositório na aba Origem (e recusar `repoUrl` solto); webhook: assinatura errada, fonte desconhecida, id que não é UUID e o segredo de outra fonte → 401; push válido dispara 1 deploy; `[skip ci]`, outra branch, outro repositório e evento que não é push disparam 0; GitLab com token errado 401 e certo dispara; excluir a fonte limpa o vínculo. Na tela: menu "Nova fonte" com GitLab/Bitbucket/Gitea, formulário por provedor, fonte listada, URL e segredo do webhook. 288 testes, `tsc` e `vue-tsc` passam.

**Não testado**: contra um GitLab/Gitea/Bitbucket de verdade (autenticação de clone com credencial: o servidor de teste não exigia login — a URL é coberta por teste unitário), Bitbucket (só o parser e o HMAC, a listagem vai pra api.bitbucket.org), webhooks reais dos três. Excluir a fonte deixa a aplicação sem credencial: o próximo deploy tenta a URL pública e falha se o repositório for privado.

## Fase 4 (parte 1): bancos — motores, versão, acesso, TLS, rede interna, healthcheck

**Motores.** Dragonfly, KeyDB e ClickHouse entraram em `DATABASE_ENGINES` (repositório da imagem, versões sugeridas, porta interna, se tem backup/TLS) e no `buildRunCommand`. ClickHouse ganha um `config.d` gerado pelo worker liberando `/backups` (senão o `BACKUP DATABASE` é recusado) e o `nofile` alto que ele pede.

**Acesso.** `public_access` (migração 0029): antes todo banco publicava a porta pro mundo sem ninguém ter pedido; agora o padrão é privado e a migração liga o acesso externo nos que já existiam, pra ninguém perder conexão. Toda aplicação e banco entra na rede `yeah-env-<ambiente>` com alias = nome do recurso. A URL de conexão (`databaseConnectionUrl`) vem interna e, se houver acesso externo, externa.

**TLS.** Node não gera certificado X.509 e a VPS não garante `openssl`, então `generateSelfSignedCert` monta o DER à mão (ECDSA P-256, SAN, sem dependência) — testado com o `X509Certificate` do próprio Node (assina, valida, a chave casa). O worker grava cert/chave na VPS e entrega a chave ao usuário que o motor usa (o uid é perguntado à própria imagem, porque muda entre alpine e debian). O PostgreSQL só **permite** TLS com `ssl=on`; pra **exigir**, um `pg_hba` gerado aceita rede só por `hostssl` e deixa o socket local livre (pg_dump).

**Testado** (painel local + VPS docker-in-docker; 7 motores criados de verdade): validação (imagem/porta/motor inválidos); os 7 ficaram `running` e **healthy**, todos **sem porta publicada**; alcançados **por nome** de um container qualquer da rede do ambiente — `psql` (42), `redis-cli` nos três, `mysql`, `mongosh` e o HTTP do ClickHouse (42); a 127.0.0.1 da VPS nada respondia; acesso externo no Postgres (porta 15432, `psql` pelo loopback) e o **conflito de porta** entre dois bancos → 409, configurações inválidas → 400 (inclusive TLS no ClickHouse); **TLS**: Postgres com `sslmode=require` conecta (`pg_stat_ssl` = true) e sem TLS `no pg_hba.conf entry … no encryption`; MySQL com cifra `TLS_AES_128_GCM_SHA256` e sem TLS "insecure transport are prohibited"; Redis e KeyDB só com `--tls` (plano: connection reset); MongoDB com `--tls` (plano recusado); Dragonfly com TLS (plano: "Bad TLS header"). Na tela: card de conexão (senha oculta/mostrar), aba "Versão, acesso e saúde", modal de criação com as versões, e os novos motores no catálogo.
...
**Testado** (painel local + VPS docker-in-docker, depois da mudança de rede): 363 testes, `tsc` e `vue-tsc`; catálogo (38 templates, 13 categorias, sem vazar o compose), validações (400), whoami acessível por `<slug>` e `<slug>-<serviço>`, stack própria com dois domínios no mesmo container (labels `own.label` preservado; só o `web` no proxy), redes conferidas (env net só com alias prefixado, sem o nome puro), alcance entre containers pelo alias, logs prefixados, stop → `stopped`/0 rodando, start → 2, restart, edição do compose + redeploy removeu o órfão `cache` e subiu `extra`, gitea com 2 containers e segredos diferentes por instância, imagem inexistente → `error` com o erro do registry no log, serviço legado intacto (lifecycle/logs 400), exclusão sem sobra de container/rede/volume e a rede do ambiente preservada. **Não testado no navegador** (telas de compose/domínios/logs, modal de novo recurso) nem com Traefik de verdade.

## Fase 4 (parte 4): backup de volume dos serviços em stack

Os volumes de um serviço são os **volumes nomeados do compose** (`volumes:` no topo; o nome no Docker é `<projeto>_<chave>`, o `name:` explícito ou o próprio nome se `external`) — `composeNamedVolumes`. Reaproveitei a tabela `volume_backups` (`owner_type=service`, a chave do volume vai no `label`, sem migração) e o mesmo job: o processor do worker ganhou um "alvo" por dono (aplicação ou serviço), com comandos de backup/restore/retenção próprios. Restore de stack para os containers (`compose stop`), substitui o volume e inicia de novo. Rotas em `serviceVolumeBackups.ts`, aba **Backups** no serviço com o aviso "volume persistente não é backup". Backup antes do primeiro deploy é recusado (409) — o container auxiliar criaria o volume vazio e o compose reclamaria dele. Excluir o serviço apaga linhas e arquivos.

**Testado** (painel + VPS docker-in-docker): volumes listados com o nome real; chave inexistente 404; segundo backup durante um em andamento 409; o tar tem o arquivo; download com magic `1f8b`; restore devolveu `version-one` e removeu o arquivo criado depois, sem tocar no outro volume (`other`), e a stack voltou a rodar; 6 backups extras → 5 linhas e 5 arquivos; outro serviço recebe 404 em restore/download/exclusão e lista vazia; serviço legado 400; restore de volume que saiu do compose 409; exclusão sem sobra de linhas nem do diretório. 365 testes, `tsc` e `vue-tsc`. **Não testado no navegador.**

## Fase 4 (parte 5): backup de volume no S3 — **não testado**

`volume_backups.s3_storage_id` (migração 0033). Ao pedir um backup (aplicação ou serviço) dá para escolher "Neste servidor" ou um destino S3 do time (validado: destino de outro time → 404). O worker arquiva no servidor, sobe pro S3 (`volumes/<dono>/<arquivo>`) e apaga a cópia local; o restore baixa do S3 para um arquivo temporário no servidor, restaura e apaga o temporário; a retenção (5) remove também o objeto; excluir o backup remove o objeto; o download redireciona para uma URL pré-assinada. As duas rotas passaram a usar `lib/volumeBackupFiles.ts` (DTO, destino, download, remoção), então a lógica de arquivo ficou num lugar só.

**Não foi rodado**: decidi parar de testar na máquina de desenvolvimento — só `tsc`, `vue-tsc` e 365 testes unitários passam. Pendente numa VPS real com bucket: backup para S3, restore a partir do S3 (arquivo temporário some), retenção apagando objetos, download pré-assinado, destino de outro time. O e2e da parte 4 (volumes locais dos serviços) foi rodado antes dessa mudança, mas o processor foi refatorado — vale repetir.
