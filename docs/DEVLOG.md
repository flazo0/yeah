...
## Primeiro teste real em VPS — dois bugs reais encontrados e corrigidos

Ganhei acesso a uma VPS Ubuntu 24.04 de verdade (Hostinger) e rodei o `install.sh` exatamente como um usuário real rodaria — `curl -fsSL .../install.sh | sudo bash`. Isso nunca tinha sido testado de ponta a ponta antes (só `bash -n`), e encontrou dois bugs reais que só apareciam fora do ambiente de desenvolvimento:
...
Depois dos dois fixes, `curl -fsSL .../install.sh | sudo bash` completo (Docker, clone, `.env`, build de todas as imagens, `docker compose up`, migrations) rodou do início ao fim numa VPS real de 1 vCPU / 3.8GB RAM sem nenhuma intervenção manual.
